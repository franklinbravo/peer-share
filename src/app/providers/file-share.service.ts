...
export class FileShareService {
  ...
  downloadsDir
  execByType: { 
    UPDATE_TITLE: (action: any) => any; 
    REMOVE_DAT: (action: any) => any; 
    TRY_ADD_DAT: (action: any) => any; 
    REQUEST_DOWNLOAD: (action: any) => any; 
    TOGGLE_PAUSE: (action: any) => any; 
    DOWNLOAD_SPARSE_DAT: (action: any) => any; 
    CANCEL_DOWNLOAD_DAT: (action: any) => any; };
  constructor(private electron:ElectronService) {
    this.middleware('Store')
    this.downloadsDir=joinPath(this.electron.remote.app.getPath('downloads'), 'PeerShare')
    this.dataDir=joinPath(homedir(),'.PeerShare')
    this.Dat= require('dat-node')
    try {
      mkdirp(this.downloadsDir)
    } catch (_) {}
    
    this.readFile = util.promisify(electron.fs.readFile)
    this.writeFile = util.promisify(electron.fs.writeFile)
    this.execByType = {
   ...
   async readJSON (file) {
    ...
    try {
      const blob = await this.readFile(file, 'utf8')
      if (!blob) {
        return {}
      }
  ...
  async loadFromDisk () {
    ...
    const [ datOpts, paused ] = await Promise.all([
      this.readJSON(joinPath(this.dataDir, 'dats.json')),
      this.readJSON(joinPath(this.dataDir, 'paused.json'))
    ])
  ...
  async internalAddDat (data) {
    let { key, path, paused, ...opts }=data
    if (key) {
...
//---------------------------------------------
    this.Dat(path, { key }, (error, dat) => {
      if (error) return this.dispatch({ type: 'ADD_DAT_ERROR', key, error })
      if (!key) {
        key = encode(dat.key)
        console.log({ type: 'ADD_DAT', key, path, paused })
        this.dispatch({ type: 'ADD_DAT', key, path, paused })
      }
      let progress
      dat.trackStats()
      if (dat.writable) progress =dat.importFiles(opts)

      progress.on('put',(src, dest)=>{
        console.log('Archivo agregado',dest.name)
      })
      this.dispatch({
        type: 'DAT_METADATA',
        key,
  ...
  async updateTitle ({ key, title }) {
    ...
    try {
      await this.writeFile(filePath, JSON.stringify(metadata))
    } catch (error) {
  ...
  async downloadSparseDat ({ key }) {
    ...
    this.Dat(path, { key, sparse: true }, (error, dat) => {
      if (error) return this.dispatch({ type: 'ADD_DAT_ERROR', key, error })
  ...
  updateProgress (dat, key, stats?) {
    ...
    if (dat.progress === 1 && unfinishedBefore) {
      ...
      notification.onclick = () =>
        this.electron.shell.openExternal(`file://${dat.path}`)
    }
...
//_______________
  ...
  //Primero ejecutamos esta funcion
  async storeOnDisk () {
    try {
      await mkdirp(this.dataDir)
    } catch (_) {}
